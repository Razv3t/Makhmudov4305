...
internal class Program
{
    ...
    private static async Task ManageUsers(IUserApi userApi)
    {
        ...
        while(true) {
            ...
            if (key.Key == ConsoleKey.D1) 
            {
                ...
                Console.WriteLine($"| Id    |      Вкус             | Статус |  Покупатель   |");
    ...
    private static void PrintMenu()
    {
        lock (_locker)
        {
            ...
            Console.WriteLine("3 - Заказать шаурму");
    ...
    static string ChoosePizza()
    {
        ...
        {
            // Набор из 5 пицц
            string[] pizzas = { "С тунцом", "С летним салатом", "Гавайская", "Недожаренная", "Вкусная(не точно)" };

            // Вывод списка пицц
            Console.WriteLine("Выберите шаурму из следующего списка:");
            ...
            Console.Write("Введите номер вкуса (1-5): ");
            int choice;

            // Проверка корректности ввода
            while (!int.TryParse(Console.ReadLine(), out choice) || choice < 1 || choice > pizzas.Length)
            {
                Console.Write("Некорректный ввод. Пожалуйста, введите номер вкуса (1-5): ");
            }
